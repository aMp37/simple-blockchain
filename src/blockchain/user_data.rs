...
impl<T> UserData<T> 
    where T: Serialize {
        ...
        fn generate_sign(&self, private_key: &RsaPrivateKey) -> RSA2048DigitalSign {
            let data_to_hash = Self::serialize_content_to_json_string(&self.content);
...
impl<T> Signable for UserData<T> 
    where T: Serialize + Debug {
        fn sign_with_private_key(&mut self, author_private_key: &RsaPrivateKey) {
            let sign = self.generate_sign(author_private_key);
